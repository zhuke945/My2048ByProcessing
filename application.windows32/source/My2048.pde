...
void drawGrid(int x,int y,int Width)
{
  for(int i=0;i<4;i++)
  {
...
void drawNumbers(int x,int y,int Width)
{
   for(int i=0;i<4;i++)
  {
    for(int j=x,k=0;k<4;j+=100,k++)
...
void drawInformationBox(int x,int y)
{
  textSize(60);
  fill(119,110,101);
  text("2048",x+5,y+10);
...
void newGameOrTryAgain(int x,int y,int Width,int Height)
{
...
void drawScores(int x,int y,int Width,int Height)
{
  fill(0xbb,0xad,0xa0);
...
void responseAfterGameOver(int x,int y,int Width,int Height)
{
  if(!flag) return;
  fill(240,123);
...
void responseAfterSucceed(int x,int y,int Width,int Height)
{
  if(succeed==1)
  {
  fill(240,123);
